...
namespace Modules.Common.Preconditions;
...
public class RequireConfirmActionAttribute : PreconditionAttribute
{
    ...
    {
        var logChannelId = await db.MafiaSettings
            .AsNoTracking()
            .Where(ms => ms.GuildSettingsId == context.Guild.Id)
            .Select(ms => ms.GeneralTextChannelId)
            .FirstOrDefaultAsync();
